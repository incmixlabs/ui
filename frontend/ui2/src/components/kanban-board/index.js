...
function convertCustomDataToBoardFormat(customData) {
...
export function Board() {
    ...
    return (_jsx(_Fragment, { children: _jsxs(Box, { className: `${kanbanFilter && "flex w-full gap-6 "} h-full overflow-hidden`, ref: scrollableRef, children: [_jsx(Box, { className: `${kanbanFilter ? "w-full space-y-5 " : "flex flex-row gap-4 2xl:gap-7"}`, children: data.columns.map((column) => (_jsx(BoardColumn, { column: column, kanbanFilter: kanbanFilter }, column.id))) }), _jsx(Suspense, { fallback: _jsx(Box, { className: "p-4", children: "Loading drawer..." }), children: _jsx(TaskCardDrawer, { kanbanFilter: kanbanFilter }) })] }) }));
}
