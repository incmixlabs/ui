 ...
 */
export default function FieldGroup({ children, layout = "column", className, columns = 2, gap = 4, }) {
    ...
    return (_jsx("div", { className: cn(layoutClasses[layout], gapClass, "w-full", className), children: React.Children.map(children, (child) => {
            if (!child)
                return null;
            // For row layout, wrap each child in a div with appropriate width
            if (layout === "row" && React.isValidElement(child)) {
                return _jsx("div", { className: "min-w-[200px] flex-1", children: child });
            }
            return child;
        }) }));
}
