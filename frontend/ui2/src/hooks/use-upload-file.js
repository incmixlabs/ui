...
export const useUploadFile = ({ types, autoUpload = true, multiple = true, opfs = { enabled: true, saveDirectory: "uploadedFiles" }, onError, onSuccess, }) => {
    ...
    const uploadToOpfs = useCallback(async () => {
        ...
        const fileHandles = await Promise.all(selectedFiles.map(async (f) => {
