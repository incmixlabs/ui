...
import type { $$ClassedProps, $$ClassedVariants, InferVariantProps, VariantConfig, Variants } from "./core";
 ...
 */
export type VariantProps<T extends InferableClassedType> = InferVariantProps<T[$$ClassedVariants]["variants"]>;
/**
 ...
 */
export type StrictComponentType<T extends AnyClassedComponent, U extends keyof VariantProps<T> | unknown = unknown, VProps extends VariantProps<T> = VariantProps<T>> = ClassedComponentType<T, Required<Pick<VProps, U extends unknown ? PickRequiredVariants<T> : U>>>;
export type PickRequiredVariants<T extends AnyClassedComponent & InferableClassedType> = T[$$ClassedVariants]["defaultVariants"] extends {} ? keyof Required<Omit<VariantProps<T>, keyof T[$$ClassedVariants]["defaultVariants"]>> : never;
/**
 * Defines a Classed component.
 */
export interface ClassedComponentType<Type extends keyof JSX.IntrinsicElements | AnyComponent, Props extends {} = {}, TComposedVariants extends {} = {}> extends Polymorphic.ForwardRefComponent<Type, Props> {
    [$$ClassedProps]: Props;
 ...
 */
export type DerivedComponentType<Type extends keyof JSX.IntrinsicElements | React.ComponentType<any>, Props extends {} = {}, TComposedVariants extends {} = {}> = ClassedComponentType<Type, Omit<Props, "as">, TComposedVariants>;
/** Returns the cumulative props from the given array of compositions. */
export type ClassedComponentProps<T extends any[]> = ($$ClassedProps extends keyof T[0] ? T[0][$$ClassedProps] : T[0] extends {
    variants: {
        [name: string]: unknown;
    };
...
export type ClassedComponentVariants<T extends any[]> = ($$ClassedVariants extends keyof T[0] ? T[0][$$ClassedVariants] : T[0] extends {
    variants: {
        [name: string]: unknown;
    };
...
export interface ClassedFunctionType {
    <Type extends keyof JSX.IntrinsicElements | AnyComponent, Composers extends (string | Util.Function | {
        base?: string;
        variants?: {
            [name: string]: unknown;
        };
        ...
        [K in keyof Composers]: string extends Composers[K] ? Composers[K] : Composers[K] extends string | Util.Function ? Composers[K] : {
            ...
            compoundVariants?: (("variants" extends keyof Composers[K] ? {
                [Name in keyof Composers[K]["variants"]]?: Util.Widen<keyof Composers[K]["variants"][Name]> | Array<Util.Widen<keyof Composers[K]["variants"][Name]>> | Util.String;
            } : never) & {
 ...
 */
export type ClassedProxyFunctionType<Type extends keyof JSX.IntrinsicElements | AnyComponent> = <Composers extends (string | Util.Function | {
    base?: string;
    variants?: {
        [name: string]: unknown;
    };
