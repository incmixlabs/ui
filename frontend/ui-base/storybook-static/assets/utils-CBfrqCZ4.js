...
const ne = "-", Re = (e) => {
  const n = Te(e), { conflictingClassGroups: r, conflictingClassGroupModifiers: o } = e;
  ...
  const m = e.join(ne);
  return (_a = n.validators.find(({ validator: l }) => l(m))) == null ? void 0 : _a.classGroupId;
}, fe = /^\[(.+)\]$/, Ge = (e) => {
  if (fe.test(e)) {
  ...
  const { theme: n, classGroups: r } = e, o = { nextPart: /* @__PURE__ */ new Map(), validators: [] };
  ...
  let o = (i) => {
    ...
    return { modifiers: m, hasImportantModifier: O, baseClassName: M, maybePostfixModifierPosition: F };
  };
  if (n) {
    const i = n + se, m = o;
    o = (l) => l.startsWith(i) ? m(l.substring(i.length)) : { isExternal: true, modifiers: [], hasImportantModifier: false, baseClassName: l, maybePostfixModifierPosition: void 0 };
  }
  if (r) {
    const i = o;
  ...
  const n = Object.fromEntries(e.orderSensitiveModifiers.map((o) => [o, true]));
  return (o) => {
...
}, Fe = (e) => ({ cache: Ne(e.cacheSize), parseClassName: Ve(e), sortModifiers: Oe(e), ...Re(e) }), _e = /\s+/, Be = (e, n) => {
  const { parseClassName: r, getClassGroupId: o, getConflictingClassGroupIds: i, sortModifiers: m } = n, l = [], u = e.trim().split(_e);
  let p = "";
  for (let g = u.length - 1; g >= 0; g -= 1) {
    const h = u[g], { isExternal: M, modifiers: O, hasImportantModifier: F, baseClassName: k, maybePostfixModifierPosition: y } = r(h);
    if (M) {
      p = h + (p.length > 0 ? " " + p : p);
      continue;
    }
...
function We() {
  let e = 0, n, r, o = "";
  for (; e < arguments.length; ) (n = arguments[e++]) && (r = ke(n)) && (o && (o += " "), o += r);
  return o;
}
const ke = (e) => {
