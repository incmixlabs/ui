...
const SAMPLE_TASKS: Task[] = [
  {
    ...
    description:
      "Update the design system components to match the latest brand guidelines",
  ...
  },
  {
    ...
    description:
      "Integrate the new payment processing API with our checkout flow",
  ...
  },
  {
    ...
    description:
      "Redesign the user onboarding process to improve conversion rates",
...
const STATUS_MAP = {
  ...
  in_review: {
    color:
      "bg-purple-50 text-purple-700 ring-purple-600/20 dark:bg-purple-950/50 dark:text-purple-400 dark:ring-purple-500/30",
  },
  done: {
    color:
...
const TASK_TABLE_COLUMNS = [
  {
    ...
    renderer: (value: string) => (
      <div className="cursor-pointer font-medium text-primary-600 hover:underline dark:text-primary-400">
        {value}
      </div>
    ),
  },
  ...
  const renderTaskDetails = useCallback(
    (task: Task) => (
      <div className="rounded-md bg-gray-50 p-4 dark:bg-gray-800">
        <h3 className="mb-2 font-semibold text-lg">{task.title}</h3>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div>
            <h4 className="mb-1 font-medium text-gray-500 text-sm">Timeline</h4>
            <p className="text-sm">
              <span className="font-medium">Start:</span>{" "}
              {new Date(task.timeline.startDate).toLocaleDateString()}
            </p>
            <p className="text-sm">
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="mb-4 font-bold text-2xl">
        Project Tasks with Timeline Progress
      </h1>
      <p className="mb-6 text-gray-600 dark:text-gray-400">
        This demo showcases the TimelineProgress column type that visualizes
        task timelines with a progress bar.
      </p>
