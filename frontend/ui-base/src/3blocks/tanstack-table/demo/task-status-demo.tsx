...
import { Box, Heading } from "@/src/1base"
...
import type { ColumnConfig } from "../components/ColumnConfigDialog"
...
const addCustomDropdownOption = (value: string): string => {
  ...
  if (processedCustomValues.has(valueLower)) {
    console.log(
      `Already processed: ${value} → ${processedCustomValues.get(valueLower)}`
    )
  ...
  const existingColors = STATUS_OPTIONS.map(
    (opt) => opt.color || "#e5e7eb"
  ).filter(Boolean)
...
const _CustomDropdownCellEditor: React.FC<{
  value: string
...
const TASK_TABLE_COLUMNS: ExtendedColumnConfig[] = [
  ...
  },
  {
    ...
    inlineCellEditor: (props: {
      ...
      console.log(
        "Column strictDropdown setting:",
        columnDef?.meta?.strictDropdown,
        "Using strictDropdown:",
        strictDropdown
      )
...
const TaskStatusDemo = () => {
  ...
  const [selectedColumn, setSelectedColumn] =
    useState<ExtendedColumnConfig | null>(null)
  const [columns, setColumns] =
    useState<ExtendedColumnConfig[]>(TASK_TABLE_COLUMNS)
  ...
  const handleSaveColumnConfig = useCallback(
    (columnId: string, updates: Partial<ExtendedColumnConfig>) => {
      ...
      if (columnIndex !== -1) {
        ...
        if (
          updates.meta?.dropdownOptions &&
          originalColumn.type === "Dropdown"
        ) {
          ...
          console.log(
            `Setting strictDropdown to: ${updates.meta.strictDropdown}`
          )
          ...
          if (columnId === "status") {
            ...
            if (updates.meta?.strictDropdown !== undefined) {
              ...
              console.log(
                "Updated STATUS_COLUMN_META.strictDropdown to:",
                STATUS_COLUMN_META.strictDropdown
              )
            }
          }
  ...
  return (
    <Box className="container mx-auto py-10">
      <Heading className="mb-4" variant="sectionTitle">
