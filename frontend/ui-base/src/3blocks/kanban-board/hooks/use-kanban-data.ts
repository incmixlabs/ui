...
export function useKanban(providedProjectId?: string): UseKanbanReturn {
  ...
  const columns = useMemo<KanbanColumn[]>(() => {
    ...
    const statusLabels = projectData.labels.filter(
  ...
  const projectStats = useMemo(() => {
    ...
    const totalStatusLabels = projectData.labels.filter(
      (label) => label.type === "status"
    ).length
    ...
    const urgentPriorityLabel = projectData.labels.find(
      (label) =>
        label.type === "priority" && label.name.toLowerCase() === "urgent"
    )

    const urgentTasks = urgentPriorityLabel
      ? projectData.tasks.filter(
          (task) =>
            task.priorityId === urgentPriorityLabel.id && !task.completed
        ).length
      : 0
