...
import type { TableTask, TaskDataSchema } from "@incmix/utils/schema"
 ...
 */
export function useTableView(providedProjectId?: string): UseTableViewReturn {
  ...
  const _kanbanData = useKanban(providedProjectId)
    ...
    const allTasks = listViewData.columns.flatMap((column) =>
      column.tasks.map((task) => {
        ...
        const priorityInfo = task.priorityId
          ? priorityMap[task.priorityId] || {
              name: task.priorityId,
              color: "#64748b",
            }
          : undefined
        ...
        return {
          ...
          completedSubTasks: enhancedSubTasks.filter((st) => st.completed)
            .length,
