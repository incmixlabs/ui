...
import { flattenColumns, isColumnGroup } from "../utils/column-utils"
import {
  dateRangeFilterFn,
  facetedFilterFn,
  textFilterFn,
...
export function useTableColumns<TData>({
  ...
  const flatColumns = useMemo(() => {
    ...
    return isColumnGroup(columns[0])
      ? flattenColumns(
          columns as (DataTableColumn<TData> | ColumnGroup<TData>)[]
        )
  ...
  const columnDefs = useMemo(() => {
    ...
    let finalDefs = defs
    if (expandableRows?.expandOnClick) {
    ...
    if (facets && facets.length > 0) {
      facets.forEach((facet: DataTableFacet<TData>) => {
        ...
        if (colDef) {
          ...
          console.warn(
            `Column with ID ${columnKey} not found for facet ${facet.title}`
          )
        }
      })
    }
    ...
    if (sidebarFilters && sidebarFilters.length > 0) {
      sidebarFilters.forEach((filter: SidebarFilterConfig<TData>) => {
        ...
        if (colDef) {
          ...
          console.warn(
            `Column with ID ${columnKey} not found for sidebar filter ${filter.title}`
          )
        }
      })
    }
