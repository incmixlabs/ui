...
// Safe wrapper for useLocation that handles cases where router context is not available
const useSafeLocation = () => {
  try {
    return useLocation()
  } catch (_error) {
    // Return a default location object when router context is not available (e.g., in Storybook)
    return { pathname: "/" }
  }
}
...
const SidebarProvider: React.FC<SidebarProviderProps> = ({
  ...
  const [_open, _setOpen] = useState(shouldAlwaysCollapse ? false : defaultOpen)
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-sidebar="group-label"
      className={cn(
        "mt-2 flex h-8 shrink-0 items-center gap-3 rounded-md px-2 font-medium text-[var(--sidebar-foreground)] text-lg outline-none ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>span:last-child]:text-2xl [&>svg]:size-4 [&>svg]:shrink-0",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:!translate-x-2.5 flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-[var(--sidebar-active)] hover:text-[var(--sidebar-foreground)] focus-visible:ring-2 active:bg-[var(--sidebar-active)] active:text-[var(--sidebar-foreground)] disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-[var(--sidebar-active)] data-[status=active]:bg-[var(--sidebar-active)] data-[active=true]:font-medium data-[active=true]:text-[var(--sidebar-foreground)] data-[status=active]:text-[var(--sidebar-foreground)] data-[state=open]:hover:bg-[var(--sidebar-active)] data-[state=open]:hover:text-[var(--sidebar-foreground)] [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 group-data-[collapsible=icon]:[&>svg]:size-5",
  {
    variants: {
      variant: {
        default:
    ...
    const button = (
      <Box className="group-data-[collapsible=icon]:my-1">
        <Comp
          ...
          {...props}
        />
        <span className="hidden w-14 text-center font-medium text-[9px] leading-tight group-data-[collapsible=icon]:block md:w-full">
          {tooltip && <>{tooltip}</>}
        </span>
      </Box>
    )
