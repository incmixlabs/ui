...
import type { LayoutGridCard } from "src/components/demos/motion/demo-layout-grid";

const LayoutGridContainer = clx.div(
  "mx-auto grid h-full w-full max-w-7xl grid-cols-1  gap-4 p-10 md:grid-cols-3",
);
const LayoutGridCardContainer = clx.div("");
      ...
      {cards.map((card, i) => (
        <LayoutGridCardContainer key={i}>
          <motion.div
            ...
            layout
          >
            {selected?.id === card.id && <GridLayoutSelectedCard selected={selected} />}
...
function GridLayoutSelectedCard({ selected }: { selected: LayoutGridCard | null }) {
  return (
