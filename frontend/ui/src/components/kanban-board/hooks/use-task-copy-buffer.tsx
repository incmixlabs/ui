...
interface CopiedTaskData {
  task: Omit<TaskDataSchema, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'updatedBy'>
  originalId: string
  originalName: string
  copiedAt: number
}
...
const TaskCopyBufferContext = createContext<TaskCopyBufferContextType | null>(null)

interface TaskCopyBufferProviderProps {
  children: React.ReactNode
}

export function TaskCopyBufferProvider({ children }: TaskCopyBufferProviderProps) {
...
export function useTaskCopyBuffer() {
  ...
  if (!context) {
    throw new Error('useTaskCopyBuffer must be used within a TaskCopyBufferProvider')
  }
