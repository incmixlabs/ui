...
import { KanbanTask, ListColumn } from "../types"
...
export function exportTasksToCSV(
  ...
  const rows = sortedTasks.map(({ task, columnName }) => {
    const row = [
      ...
      escapeCSVField(task.startDate ? new Date(task.startDate).toLocaleDateString() : ''),
      escapeCSVField(task.endDate ? new Date(task.endDate).toLocaleDateString() : ''),
    ...
    if (options.includeSubtasks) {
      row.push(
        ...
        escapeCSVField(task.parentTaskId ? findParentTaskName(allTasks, task.parentTaskId) : '')
      )
    }
 ...
 */
export function downloadCSV(csvContent: string, filename: string = 'tasks-export.csv'): void {
 ...
 */
export function exportColumnToCSV(column: ListColumn, priorityLabels: LabelSchema[] = [], options?: CSVExportOptions): void {
 ...
 */
export function exportAllTasksToCSV(columns: ListColumn[], priorityLabels: LabelSchema[] = [], options?: CSVExportOptions): void {
...
function formatAssignees(assignees: Array<{name?: string, id?: string}>): string {
  return assignees.map(assignee => assignee.name || assignee.id || '').join('; ')
}
...
function findParentTaskName(allTasks: Array<{task: KanbanTask}>, parentId: string): string {
  const parentTask = allTasks.find(({ task }) => task.id === parentId)
  return parentTask?.task.name || parentId
}

function resolvePriorityName(priorityId: string | undefined, priorityLabels: LabelSchema[]): string {
  if (!priorityId) return ''
  
  // Find the priority label by ID
  const priorityLabel = priorityLabels.find(label => 
    label.type === 'priority' && label.id === priorityId
  )
