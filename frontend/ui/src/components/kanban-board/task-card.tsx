...
import { getCardData, getCardDropTargetData, isCardData, isDraggingACard, TCard } from './types';
...
export function CardDisplay({
  ...
  return (
    ...
    >
      ...
      >
        <span className="absolute right-3 top-2 flex items-center gap-1 text-sm font-medium text-black">
        ...
        {card.description && <p className="text-zinc-600">{card.description}</p>}
        {card.subTasks &&
          card.subTasks.map((subtask) => (
            <>
              <div className="gap-2 space-y-2 py-2">
                <div className="flex w-full items-center justify-between gap-1 uppercase text-zinc-500">
                  <span>{subtask.name}</span>
                  <span>{subtask?.progress}%</span>
                </div>
                <div
                  className="relative flex h-2 w-full items-center gap-1 rounded-full bg-gray-200 before:absolute before:left-0 before:top-0 before:h-2 before:w-[var(--progress)] before:rounded-full before:bg-green-400"
                  style={{ '--progress': `${subtask?.progress}%` } as React.CSSProperties}
                />
              </div>
            </>
          ))}
        ...
        <div className="flex items-center justify-between gap-2 pb-2 pt-6">
