...
export function useTableEdit<TData>({
  ...
  const handleEditClick = useCallback((row: TData) => {
  ...
  const handleEditSubmit = useCallback((oldData: TData, newData: TData) => {
    if (onRowEdit) {
  ...
  const enhancedRowActions = useCallback((row: TData): RowAction[] => {
